...
- (NSString *)getLicenseString {
	//PhotoPickerAppDelegate *appDelegate = (PhotoPickerAppDelegate *) [UIApplication sharedApplication].delegate;
	NSArray *licenses = [[NSArray alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Licenses" ofType:@"plist"]];
    NSString *licenseDefault = [[NSUserDefaults standardUserDefaults] stringForKey:LICENSE_KEY];
    ...
    if( aLicense != nil ) {
        ...
        licenseString = [licenseString stringByReplacingOccurrencesOfString:@"$author" withString: [[NSUserDefaults standardUserDefaults] valueForKey: USERNAME_KEY]];
...
- (NSString *)getUploadDescription {
    ...
    NSString *result = [NSString stringWithFormat: @"{{Information\n|Description={{en|1=%@}}\n|Author=[[User:%@]]\n|Source={{own}}\n|Date=%@\n|Permission=\n|other_versions=\n}}\n\n== {{int:license}} ==\n%@\n\n[[Category:%@]]", self.description, [[NSUserDefaults standardUserDefaults] stringForKey:USERNAME_KEY], dateString, [self getLicenseString], APP_CATEGORY];
     NSLog( @"%@", result );
     return result;
}
...
- (void)uploadImage {
    ...
    NSError *error = nil;
    NSString *password = [SFHFKeychainUtils getPasswordForUsername:username andServiceName:KEYCHAIN_KEY error:&error];
